...
from utils.analyzer import analyze_inventory, analyze_shipping, build_data_summary
from utils.visualizations import (
    inventory_quantity_chart,
    shipping_status_chart,
    shipping_delay_chart,
)
from utils.ai_engine import ask_question, summarize_contract, AIEngineError
...
# Session state
# ---------------------------------------------------------------------------
for key in ("inventory_df", "shipping_df", "contract_text", "chat_history"):
...
# Tabs
# ---------------------------------------------------------------------------
tab_inventory, tab_shipping, tab_contract, tab_dashboard, tab_ai = st.tabs(
    ["📦 Inventory", "🚚 Shipping", "📄 Contract", "📊 Dashboard", "🤖 Ask AI"]
)
...
with tab_disruption:
    ...
    with col_b:
        ...
        disruption_reason = st.text_area(
            "What happened?",
            placeholder="e.g. Port strike blocking shipments from this region",
            height=100,
        )
    ...
    if st.button("🚨 Generate Response Plan", type="primary"):
        if not item_name:
            ...
            if not alternatives.empty:
                ...
                st.info("No alternative suppliers found for this item in the directory.")
            ...
            try:
                with st.spinner("Working out the best response..."):
                    plan = recommend_disruption_response(
                        context, api_key=get_api_key(), model=get_model()
                    )
